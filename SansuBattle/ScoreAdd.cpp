...
void ScoreAdd::CalcScore(int gamelevel, int time)
{
	...
	{
		addscore_value = SCOREADD_SCORE_ANSER * TIME_BONUS_EXCELLENT;	//エクセレントのボーナスを加えて計算
	}
	else if (time <= time_great)//グレートの評価基準より短ければ
	{
		addscore_value = SCOREADD_SCORE_ANSER * TIME_BONUS_GREAT;		//グレートのボーナスを加えて計算
	}
	else						//それより遅ければ
	{
		addscore_value = SCOREADD_SCORE_ANSER;	//ボーナスなしで計算
	}
